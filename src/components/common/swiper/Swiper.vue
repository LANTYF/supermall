...
<template>
  <div class="h_swiper">
    ...
    <div class="dots">
      <span class="dot" v-for="(item,index) in slideCount" :key="index" :class="{active:index === isShow}"></span>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    handledom() {
      ...
      let swiperitem = document.getElementsByClassName("swiperitem");
    ...
    touchStart(e) {
      if(this.scrolling){return};//在自动播放时禁止滑动
    ...
    touchEnd(e) {
      ...
      else if(this.distance < 0 && absDistance > this.totalWidth * this.moveRatio){//向左滑动距离超过0.25倍的宽度
        this.currentIndex++
      }
      else if(this.distance > 0 && absDistance > this.totalWidth * this.moveRatio){//向右滑动距离超过0.25倍的宽度
        this.currentIndex--
      }
      this.setScrollContent(this.currentDistance)
    }
